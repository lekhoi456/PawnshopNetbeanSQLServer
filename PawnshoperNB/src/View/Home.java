...
import static View.Login.userNameLogged;
...
public class Home extends javax.swing.JFrame {
    ...
    private void initpnlDashboard() throws Exception {
        ...
        totalCashFund = storeModel.getList().get(0).getInvestmentCapital();
        moneyInvestment = 0;
        totalInterestEarn = 0;
        for (int i = 0; i < contractModel.getList().size(); i++) {
            if (contractModel.getList().get(i).getStatus() == 1) {
                totalCashFund += contractModel.getList().get(i).getTotalMoney();
                totalInterestEarn += contractModel.getList().get(i).getTotalMoney() - contractModel.getList().get(i).getTotalLoanAmount();
            } else {
        ...
        for (int i = 0; i < storeModel.getList().size(); i++) {
            lblTotalCashFund.setText(Validation.convertLongFormat(totalCashFund));
        ...
        try {
            storeModel.updateMoney(1, totalCashFund, totalInterestEarn);
...
//    }
    ...
    private void initComponents() {
        ...
        pnlCardDashboard.add(jLabel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(950, 10, 30, 20));
    ...
    private void tblContractMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblContractMousePressed
        cbContractId.setSelectedItem(tableContract.getValueAt(tblContract.getSelectedRow(), 0).toString());
//        System.out.println("Select CB: " + cbContractId.getSelectedItem().toString());
//        System.out.println("Select contract ID: " + tableContract.getValueAt(tblContract.getSelectedRow(), 0));
    }//GEN-LAST:event_tblContractMousePressed

    private void tblCustomerMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblCustomerMousePressed
        cbSelectCustomerId.setSelectedItem(tableCustomer.getValueAt(tblCustomer.getSelectedRow(), 0).toString());
    ...
    private void tblEmployeeMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblEmployeeMousePressed
        cbSelectEmployee.setSelectedItem(tableEmployee.getValueAt(tblEmployee.getSelectedRow(), 0).toString());
...
//    }
    ...
    private static long totalCashFund;
    private static long totalInterestEarn;
    private static long moneyInvestment;
